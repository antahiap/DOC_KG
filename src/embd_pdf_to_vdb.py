...
from constants import PAPER_REF_PATH, OPEN_AI, OLLAMA, OLLAMA_URL, LLM_MODEL, EMBED_DIR
...
class Vectordb():
    def __init__(self, file_path, doc_id, read_json=False, file_read=False):
        self.db_index = []
        self.sql = DB.Database()

        self.doc_id = doc_id
        self.embd_path = f'{EMBED_DIR}/embeddings_{doc_id}.pkl'
        
        file_name =  file_path / Path(doc_id) #.pdf')
        file_reader = FileReader(file_name, write_json=True)
        json_path = file_reader.get_json_path()


        if file_read:
            with open(file_read, 'r') as f:
                self.data = json.load(f)
            print(f"Loading data from {file_read}")

        elif os.path.isfile(json_path) and read_json:
            with open(json_path, 'r') as f:
                self.data = json.load(f)
        else:
            self.data = file_reader.read_pdf()
    ...
    def embd_sec(self, readOn=True, nsec=2):
        ...
        if not readOn or not os.path.isfile(self.embd_path):
            ...
            if OLLAMA:
                ...
                sec_result = client.embed(model=LLM_MODEL, input=self.texts[:nsec]).embeddings
...
if __name__ == '__main__':
    ...
    paper_id = "1903.10174v1"
    paper_id = "1706.03762.pdf"
    
    vdb =Vectordb(src, paper_id)
    emb = vdb.embd_sec(readOn=False, nsec=2)
